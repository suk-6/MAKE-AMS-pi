...
def getCodes():
    res = requests.get(
        f"{apiUrl}/admin/code-list", params={"admin": ADMIN_KEY}, timeout=5
    )
...
def checkAccess(code=None):  # QR 코드 문자열을 매개변수로 받음 (기본값: None)
    ...
    try:  # 예외 처리 시작 (HTTP 요청 중 오류 발생 가능성 대비)
        if code is None:  # QR 코드가 제공되지 않은 경우
            res = requests.get(
                f"{apiUrl}/auth/access", timeout=5
            )  # QR 코드 없이 인증 요청
        else:  # QR 코드가 제공된 경우
...
def saveAccess(code):
    try:
        requests.get(  # QR 코드를 파라미터로 추가하여 인증 요청
            f"{apiUrl}/auth/access", params={"code": code}, timeout=5
        )
    except:
        pass
